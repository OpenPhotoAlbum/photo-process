...
async function freshStartCleanup() {
    try {
        ...
        if (!keepCompreFace) {
            ...
            try {
                // Get all subjects
                const subjectsResponse = await fetch('http://localhost:8000/api/v1/recognition/subjects', {
                ...
                if (subjectsResponse.ok) {
                    ...
                    for (const subject of subjects) {
                        try {
                            const deleteResponse = await fetch(`http://localhost:8000/api/v1/recognition/subjects/${encodeURIComponent(subject)}`, {
                                method: 'DELETE',
                                headers: { 'x-api-key': 'b6dd9990-6905-40b8-80d3-4655196ab139' }
                            });
        ...
        if (!keepCompreFace) {
            try {
                const subjectsCheck = await fetch('http://localhost:8000/api/v1/recognition/subjects', {
                    headers: { 'x-api-key': 'b6dd9990-6905-40b8-80d3-4655196ab139' }
                });
