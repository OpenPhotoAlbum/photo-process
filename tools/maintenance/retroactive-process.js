...
const { detectObjects } = require('./build/api/util/object-detection');
const { extractFaces } = require('./build/api/util/compreface');
const { ImageRepository, ObjectRepository, FaceRepository } = require('./build/api/models/database');
...
const MEDIA_SOURCE_DIR = process.env.media_source_dir || '/mnt/sg1/uploads/stephen/iphone';
...
async function findImagesWithoutObjects(limit) {
    ...
    const { db } = require('./build/api/models/database');
...
async function updateJsonMetadata(image, newData) {
    try {
        // Find the corresponding JSON metadata file
        const metadataPath = path.join(MEDIA_DEST_DIR, 'recents', 'meta', `${image.filename}.json`);
        ...
        console.error(`   ⚠️  Failed to update metadata file:`, error.message);
    }
}
