...
export class StartupValidator {
    ...
    private async validateConfiguration(): Promise<void> {
        try {
            ...
            const logsDir = storage.logsDir || './logs';
            if (!fs.existsSync(logsDir)) {
                try {
                    fs.mkdirSync(logsDir, { recursive: true });
                    this.addResult('Configuration', 'Log Directory', true, 
                        `Created log directory: ${logsDir}`);
                } catch (error) {
                    this.addResult('Configuration', 'Log Directory', false, 
                        `Cannot create log directory: ${logsDir}`, true);
                }
            } else {
                this.addResult('Configuration', 'Log Directory', true, 
                    `Log directory exists: ${logsDir}`);
            }
    ...
    private async validateDatabase(): Promise<void> {
        try {
            ...
            // Don't mark as critical during development - log but continue
            this.addResult('Database', 'Connection', false, 
                `Database connection failed: ${error instanceof Error ? error.message : error}`, false);
        }
    }
    ...
    private async validateCompreFace(): Promise<void> {
        try {
            ...
            if (!comprefaceConfig.baseUrl) {
                this.addResult('CompreFace', 'Configuration', false, 
                    'CompreFace baseUrl not configured', true);
                return;
            }

            // Test if CompreFace is accessible
            const response = await fetch(comprefaceConfig.baseUrl, { 
                timeout: 5000,
                method: 'HEAD'
            });
            ...
            if (response.ok) {
                this.addResult('CompreFace', 'Service Access', true, 
                    `CompreFace service is accessible at ${comprefaceConfig.baseUrl}`);

                // Test API endpoint with a simple request
                try {
                    const apiResponse = await fetch(`${comprefaceConfig.baseUrl}/api/v1/detection/detect`, {
                        method: 'POST',
